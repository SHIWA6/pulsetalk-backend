...
async function getMessagesForRoom(room) {
    try {
        ...
        const messagesFromDB = await prisma_server_1.default.chatMessage.findMany({
...
function setupSocket(io) {
    ...
    io.on("connection", async (socket) => {
        ...
        if (socket.room) {
            try {
                const group = await prisma_server_1.default.chatGroup.findUnique({
            ...
            console.log("Current sockets in room:", Array.from(io.sockets.adapter.rooms.get(socket.room) || []));
        ...
        socket.on("send_message", async (data, callback) => {
            ...
            try {
                const user = await prisma_server_1.default.user.findUnique({
                    where: { email: userInfo.email },
                });
                if (!user)
                    throw new Error(`User with email ${userInfo.email} not found`);
                const savedMessage = await prisma_server_1.default.chatMessage.create({
                    data: {
